...
if($conn==false){
  ...
  $y_Phase_R   = mysqli_query($conn, 'SELECT daya_R FROM ( SELECT * FROM datalog_listrik WHERE perangkat_id = 101 ORDER BY ID DESC LIMIT 25) Var1 ORDER BY ID ASC'); 
  $y_Phase_S   = mysqli_query($conn, 'SELECT daya_S FROM ( SELECT * FROM datalog_listrik WHERE perangkat_id = 101 ORDER BY ID DESC LIMIT 25) Var1 ORDER BY ID ASC'); 
  $y_Phase_T   = mysqli_query($conn, 'SELECT daya_T FROM ( SELECT * FROM datalog_listrik WHERE perangkat_id = 101 ORDER BY ID DESC LIMIT 25) Var1 ORDER BY ID ASC');
  ?>
       ...
       var canvas = document.getElementById('myChart');
        var data = {
            ...
            },
            {
                label: "Phase_T", 
                fill: true,
                lineTension: 0.2 "rgba(34, 40, 34, .2)",
